...
namespace Nglib.FORMAT
{
    ...
    public class FormulaTests
    {
        ...
        // liste de formules et le résultat attendu
        private static Dictionary<string, string> GetFormulaTests()
        {
            Dictionary<string, string> formules = new Dictionary<string, string>();
            formules.Add("@MaChaine", "HelloWorld!!!");
            formules.Add("@MonNombre+4", "123460"); // Paramètre direct, donc addition
            formules.Add("'AB'+'CD'", "ABCD");
            formules.Add("4+5", "9");
            formules.Add("'4'+5", "45"); // MODE STRICT : string + number = concaténation
            formules.Add("'4'+'5'", "45");
            ...
            formules.Add("if(lt(data('@MonPo','Age'), 18), 'Mineur', if(lt(data('@MonPo','Age'), 65), 'Adulte', 'Senior'))", "Adulte");
            formules.Add("add(mul(data('@MonPo','Age'), 2), div(120000, 1000))", "170");
            
            // Conditions imbriquées avec calculs
            formules.Add("if(and(gt(data('@MonPo','Age'), 18), lt(data('@MonPo','Age'), 60)), add(data('@MonPo','Age'), 5), sub(data('@MonPo','Age'), 5))", "30");
            formules.Add("if(or(eq(upper(data('@MonPo','Nom')), 'TOTO'), eq(data('@MonPo','Age'), 25)), 'Match', 'NoMatch')", "Match");
            ...
            formules.Add("upper('hello') + ' ' + lower('WORLD')", "HELLO world");
            formules.Add("left(@MaChaine, 5) + right(@MaChaine, 3)", "Hello!!!");
            formules.Add("data('@MonPo','Nom') + ' ' + data('@MonPo','Nom')", "Toto Toto");
            ...
            formules.Add("upper(data('@MonPo','Nom')) + ' - Age: ' + data('@MonPo','Age')", "TOTO - Age: 25");
            
            // Additions pures (tous numériques = addition)
            formules.Add("5 + 3", "8");
            formules.Add("10 + 20", "30");
            formules.Add("data('@MonPo','Age') + 5", "30");
            ...
            formules.Add("'Bonjour ' + upper(data('@MonPo','Nom')) + ', vous avez ' + data('@MonPo','Age') + ' ans!'", "Bonjour TOTO, vous avez 25 ans!");
            formules.Add("left(@MaChaine, 5) + ' - ' + if(gt(len(@MaChaine), 10), 'Long', 'Court') + ' - ' + right(@MaChaine, 3)", "Hello - Long - !!!");
            formules.Add("'[' + upper(left(data('@MonPo','Nom'), 2)) + '] Age: ' + data('@MonPo','Age')", "[TO] Age: 25");
            ...
            formules.Add("'Date: ' + todate(data('@MonPo','DateLecture'),'dd/MM/yyyy')", "Date: 06/10/2020");
            formules.Add("upper(data('@MonPo','Nom')) + ' - ' + todate(data('@MonPo','DateLecture'),'yyyy')", "TOTO - 2020");
            ...
            formules.Add("left(upper(data('@MonPo','Nom')), add(1, 2))", "TOT");
            
            // Logique booléenne complexe (3-4 niveaux)
            formules.Add("and(gt(len(@MaChaine), 10), and(lt(data('@MonPo','Age'), 30), eq(upper(data('@MonPo','Nom')), 'TOTO')))", "1");
            ...
            formules.Add("if(eq(month(data('@MonPo','DateLecture')), 10), day(data('@MonPo','DateLecture')), 0)", "6");
            
            // Conversion avec imbrications
            formules.Add("if(IsNumeric(param('@MonNombre')), add(ToInt(param('@MonNombre')), 1000), 0)", "124456");
            ...
            formules.Add("upper(left(if(gt(data('@MonPo','Age'), 20), data('@MonPo','Nom'), 'Unknown'), len(data('@MonPo','Nom'))))", "TOTO");
            ...
            formules.Add("if(Between(data('@MonPo','Age'), sub(30, 10), add(20, 10)), upper('ok'), lower('NOK'))", "OK");
