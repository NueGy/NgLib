...
namespace Nglib.FILES.SERIAL
{
    ...
    public static class CsvTools
    {
        ...
        */
        ...
        /// </summary>
        public static string[] SplitCsv(string line, bool TrimAndClean = true, bool toUpper = false, char separator =DefaultCsvSeparator)
        {
            if (line == null) return null;
            if (string.IsNullOrWhiteSpace(line)) return new string[0];
            string[] retour = line.Split(separator, StringSplitOptions.None).ToArray();
            if (TrimAndClean)
                retour = retour.Select(x => x.Replace("\r", "").Replace("\n", "").Replace("\t", "")
                                            .Trim().Trim('"').Trim('\'').Trim()).ToArray();
        ...
        /// </summary>
        public static string[] SplitValues(string line, bool toUpper=true,char separator = DefaultCsvSeparator) => SplitCsv(line, true, toUpper, separator).Where(t=>!string.IsNullOrWhiteSpace(t)).ToArray();
