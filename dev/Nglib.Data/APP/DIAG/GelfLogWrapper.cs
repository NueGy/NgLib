...
namespace Nglib.APP.DIAG
{
    ...
    public class GelfLogWrapper
    {
        ...
        {
            udpClient = new System.Net.Sockets.UdpClient(host, port);
        ...
        public async Task<bool> SendAsync(Dictionary<string, object> message)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(OvhToken) && !message.ContainsKey("_X-OVH-TOKEN"))
                    message.Add("_X-OVH-TOKEN", OvhToken);
                if (!string.IsNullOrEmpty(NgyToken) && !message.ContainsKey("_X-NGY-TOKEN"))
                    message.Add("_X-NGY-TOKEN", NgyToken);
                ...
                bytes = Encoding.UTF8.GetBytes(serializedGelfMessage);


                // limit size : 8192 bytes


                int ret = await udpClient.SendAsync(bytes, bytes.Length);
        ...
        /// Gzips a string
        /// </summary>
        public static byte[] ConvertGzip(string message, Encoding encoding)
        {
